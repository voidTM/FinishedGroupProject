...
/*
	Revisions made
		Changed Hash function to spread things out.
		Made the initializing part of the hash table dynamic.
*/
#include "HashClass.h"
#include "BST.h"
...
int HashClass::HashFunction(string key) {
    ...
    for (int index = 0; index < key.length(); index++) {
		//Attempts to spred out the index by multiplying the ascii value by the square roots
        hash = hash + (int) key[index] * sqrt(tableSize * index); 
        // in the string	<-current best
    };
